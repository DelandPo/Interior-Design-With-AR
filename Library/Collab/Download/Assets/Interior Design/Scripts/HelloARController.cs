...
namespace GoogleARCore.HelloAR
{
    ...
    public class HelloARController : MonoBehaviour
    {
        ...
        public void moveCurrentSelectedObject()
        {
            
            andyObject.transform.Rotate(Vector3.up * 90, Space.World);

        }

        IEnumerator LoadVermeerScene()
        {
            AsyncOperation asyncLoadVermeer = SceneManager.LoadSceneAsync(1);

            while (!asyncLoadVermeer.isDone)
            {
                yield return null;
            }
        }

        public void loadVermeer()
        {
            StartCoroutine(LoadVermeerScene());
        }

        public void loadInterior()
        {
            StartCoroutine(LoadInteriorDesign());
        }

        IEnumerator LoadInteriorDesign()
        {
            AsyncOperation asyncLoadInteriorDesign = SceneManager.LoadSceneAsync(0);

            while (!asyncLoadInteriorDesign.isDone)
            {
                yield return null;
            }
        }
